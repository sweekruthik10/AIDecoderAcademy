...
type Kind = "worksheet" | "audio" | "image";
...
export async function POST(req: Request) {
  ...
  if (kind !== "worksheet" && kind !== "audio" && kind !== "image") {
    return NextResponse.json({ error: "Missing or invalid ?kind= (worksheet|audio|image)" }, { status: 400 });
  }
  ...
  if (file.size > MAX_BYTES) return NextResponse.json({ error: "File too large (max 60MB)" }, { status: 413 });

  const ext = (file.name.split(".").pop() ?? "").toLowerCase().replace(/[^a-z0-9]/g, "");
  const allowedByMime =
    kind === "worksheet" ? WORKSHEET_MIME.has(file.type) :
    kind === "audio"     ? AUDIO_MIME.has(file.type)     :
                           IMAGE_MIME.has(file.type);
  const allowedByExt =
    kind === "worksheet" ? (ext === "pdf" || ext === "docx") :
    kind === "audio"     ? ["mp3", "wav", "ogg", "flac", "aac", "m4a"].includes(ext) :
                           ["png", "jpg", "jpeg", "webp", "gif"].includes(ext);

  if (!allowedByMime && !allowedByExt) {
    const accept =
      kind === "worksheet" ? ".pdf or .docx" :
      kind === "audio"     ? ".mp3/.wav/.ogg/.flac/.aac/.m4a" :
                             ".png/.jpg/.webp/.gif";
  ...
  const path     = `obj8-submissions/${userId}/${kind}-${stamp}-${baseName}.${ext}`;

  const supabase = createAdminClient();
  const { error } = await supabase.storage.from("creations-media").upload(path, buffer, {
    contentType: file.type || (kind === "worksheet" ? "application/octet-stream" : kind === "audio" ? "audio/mpeg" : "image/png"),
    upsert: false,
  });

  if (error) {
    console.error("[obj8-upload] supabase upload error:", error);
