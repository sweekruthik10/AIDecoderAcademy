...
// Build a human-readable field-mapping block from the schema so the LLM knows
// exactly which bold label in the DOCX corresponds to which field ID.
...
export async function POST(req: Request) {
  try {
    ...
    // Only DOCX is supported here; PDF could be added later.
    if (body.format !== "docx") {
      return jsonOk({});
    }

    // ── 1. Fetch and extract DOCX ──────────────────────────────────────────
    let docText: string;
    try {
      ...
      return jsonOk({});   // graceful — caller treats empty data as "nothing to parse"
    }

    // ── 2. LLM extraction with schema-aware label mapping ──────────────────
